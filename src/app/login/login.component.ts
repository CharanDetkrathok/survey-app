...
export class LoginComponent implements OnInit {
  ...
  async getCallingApiService() {
    ...
    if (this.isTH_EN) {
      ...
      await this.loginService.httpCheckLanguageBefore(this.STD_CODE).subscribe(response => {

        const str = response.FIRST_NAME.toLowerCase();
        const regexp = /[ก-ฮ]/i;
        const matches_array = str.match(regexp);

        if (matches_array === null && str !== '') {

          //-- ********** สำเร็จ แจ้งด้วย Dialog และจบการทำงาน Redirect to Login ******** --//
          const title = 'Login Fail';
          const message_insert = ``;
          const message = 'Please select the language that corresponds to the survey. That you have recorded';
          const description = 'If you want to edit or want to view the details of your survey, you can do so by logging in again.';
          ...
          this.loginService.httpLogin(this.STD_CODE, this.BIRTH_DATE).subscribe(response => {
      ...
      await this.loginService.httpCheckLanguageBefore(this.STD_CODE).subscribe(response => {
        ...
        if (matches_array === null && str !== '') {
          ...
          this.loginService.httpLoginEn(this.STD_CODE, this.BIRTH_DATE).subscribe(response => {
              ...
              if (this.dialog_confirm_result) {
                ...
                if (response.error_message_status == 1) {
                  ...
                  if (response.LEV_ID == '1') {
                    ...
                    //-- ไปยังหน้า แบบสำรวจ ป.ตรี
                    this.router.navigate(['/question-naire-en']);

                  } else {

                    //-- ไปยังหน้า แบบสำรวจ ป.โท
                    this.router.navigate(['/question-naire-en-m']);
            ...
            sessionStorage.setItem('loginUnSuccessesSessionStorage', error.status);
            this.handlesErrorsEn(error.status);
  ...
  public handlesErrorsEn(_handle_error: any): void {
